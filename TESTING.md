...
**Speed:** ~1 minute for all 222 tests.
...
| `tests/pages.spec.js` | committee.html, races.html (mode cards, curated form), race.html (candidate cards, financial figures, cycle-anchored links), candidates.html (auto-load, search input, clean URLs, filter chips, URL sync, error state, ?q= search mode), committees.html (auto-load, clean URLs, filter chips, URL sync, error state, ?q= search mode), process-log.html, design-system.html (token tables, color swatches, component card attributes), index.html redirect, mobile layout at 390px and 1280px |
